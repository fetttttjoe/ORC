...
export type RankedNeighbor = { id: string; via: LinkKind; depth: number; score: number; direction: EdgeDirection; from: string }
  ...
  for (let d = 1; d <= depth && frontier.length; d++) {
    ...
    for (const f of frontier) for (const e of adj.get(f.id) ?? []) {
      ...
      if (!prev || sd > prev.score || (sd === prev.score && (d < prev.depth || (d === prev.depth && e.kind < prev.via)))) {
        best.set(e.to, { id: e.to, via: e.kind, depth: d, score: sd, direction: e.direction ?? EDGE_DIRECTION.out, from: e.from })
